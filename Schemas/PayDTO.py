from Schemas.BaseDTO import BaseDTO
from typing import Optional
from decimal import Decimal

class PayDTO(BaseDTO):
    PAY_Idx : Optional[int] = None
    MEM_Idx : Optional[int] = None
    MUR_Idx : Optional[int] = None
    PAT_Idx : Optional[int] = None
    CST_Idx : Optional[int] = None
    PAY_Status : Optional[str] = None
    PAY_AMOUNT_TOT : Optional[Decimal] = None
    PAY_AMOUNT_INSURED : Optional[Decimal] = None
    PAY_AMOUNT_NONINSURED : Optional[Decimal] = None
    PAY_AMOUNT_PATIENT : Optional[Decimal] = None
    PAY_AMOUNT_PAID : Optional[Decimal] = None
    PAY_AMOUNT_REMAIN : Optional[Decimal] = None
    PAY_Date : Optional[str] = None
    PAY_YYMMDD : Optional[str] = None
    PAY_IsValid : Optional[bool] = None

class Pay_Req(PayDTO):
    pass

class Pay_Res(PayDTO):
    pass

